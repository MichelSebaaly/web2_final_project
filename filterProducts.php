...
<body style="margin:0px;overflow-x:hidden;">
	...
	<div class="productsFiltersDiv">
		<?php
		// echo $_GET['cat'];
		$con = mysqli_connect("localhost", "root", "", "web3") or die("unable to connect");
		$query = "SELECT * FROM products WHERE ((type_id IN (SELECT id FROM product_type WHERE name='" . $_GET['cat'] . "'))AND (archived_at IS NULL))";
		$res = mysqli_query($con, $query);
		if (mysqli_num_rows($res) > 0) {
			while ($rslt = mysqli_fetch_assoc($res)) {
				echo "<div class='productsFiltersDivOfProduct' onclick='selectThisProduct(\"" . $rslt['id'] . "\");'>";
				echo "<img class='productsFiltersDivOfProductImage' src='images/" . $rslt['image'] . "'/><br>";
				echo "<div class='productsFiltersDivOfProductName'><b>" . $rslt['name'] . "</b></div><br><br>";
				echo "<div class='productsFiltersDivOfProductPrice'>" . $rslt['price'] . " L.L.</div><br><br>";
				echo "<div class='productsFiltersDivOfProductDivButton'><button class='productsFiltersDivOfProductButton'>More</button></div>";
				echo "</div>";
			}
		}
		mysqli_close($con);
