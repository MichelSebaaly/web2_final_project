 ...
 <body style="margin:0px;overflow-x:hidden;">
 	...
 	<div class="navDiv">
 		...
 		<div class="navDivMyBag" id="HomePageMyBagId">
 			...
 			<div class="MyBagDivToApp" id="MyBagDivToAppId">
 				<?php
					if ($loggedIn == false) {
 	...
 	<div class="selectedProductDiv">
 		<div class="selectedProductDivLeft">
 			<?php
				$con = mysqli_connect("localhost", "root", "", "web3") or die("unable to connect");
				$query = "SELECT * FROM products WHERE id = '" . $_GET['prodId'] . "'";
				$res = mysqli_query($con, $query);
				if (mysqli_num_rows($res) > 0) {
					$rslt = mysqli_fetch_assoc($res);
					echo "<img class='selectedProductDivLeftImg' src='images/" . $rslt['image'] . "'/>";
				?>
 		</div>
 		<div class="selectedProductDivRight">
 			<?php
					echo "<h2>" . $rslt['name'] . "</h2>";
					echo "<h2 class='selectedProductDivRightPrice'>" . $rslt['price'] . " L.L.</h2>";
					// }
					// mysqli_close($con);
				?>
 		...
 		<?php
					$con = mysqli_connect("localhost", "root", "", "web3") or die("unable to connect");
					$queryQttChck = "SELECT Quantity FROM stock WHERE product_id = '" . $_GET['prodId'] . "'";
					$resQttChck = mysqli_query($con, $queryQttChck);
					$fetchQttChck = mysqli_fetch_assoc($resQttChck);
					$qttValue = 1;
					$descrpt = "";

					if ($fetchQttChck['Quantity'] > 0) {
						$descrpt = "<h3>In Stock : " . $fetchQttChck['Quantity'] . "</h3>" . $rslt['description'];
					} else {
						$qttValue = 0;
						$descrpt =  "<h3>Out of Stock</h3>" . $rslt['description'];
					}
				}
				mysqli_close($con);
			?>

 		<div class="selectedProductDivRightQtt">
 			<div class="selectedProductDivRightQttDivLabel"><label>Quantity :</label></div>
 			<div class="selectedProductDivRightQttDivInput">
 				<button id="selectedProductDivRightQttLabelMinusId" class="selectedProductDivRightQttButtonMinus" onclick="minusQuantityProd();">-</button>
 				<input type="number" id="selectedProductDivRightQttInputId" class="selectedProductDivRightQttInput" min="1" value="<?php echo $qttValue; ?>" readonly />
 				<button id="selectedProductDivRightQttButtonPlusId" class="selectedProductDivRightQttButtonPlus" onclick="plusQuantityProd('<?php echo $_GET['prodId']; ?>');">+</button>
 			</div>
 			<div class="selectedProductDivRightQttDivButton" onclick="addThisProduct('<?php echo $rslt['id']; ?>');">
 				<label class="selectedProductDivRightQttDivButtonLabel">Add to cart</label>
 			</div>
 		</div>

 		<div class="selectedProductDivRightDescription">
 			<h2 class="selectedProductDivRightDescriptionTitle">Description</h2>
 			<?php
				echo $descrpt;
				?>
 		</div>
 	...
 	<script>
 		...
 		function addThisProduct(productId) {
 			...
 			var quantity = document.getElementById("selectedProductDivRightQttInputId").value;

 			<?php
				if (!$loggedIn || !(isset($_SESSION['AllMyProducts']))) {
				?>
 				canContinue = false;
 			<?php
				}
 			...
 			if (canContinue) {
 				//parse : from json object to js object
 				//encode : array to json object
 				myProductsId = JSON.parse('<?php
											if (isset($_SESSION['AllMyProducts'])) {
												echo json_encode($_SESSION['AllMyProducts']);
											} ?>');


 				for (var i = 0; i < myProductsId.length; i++) {
 					if (myProductsId[i] == productId) {
 			...
 			if (canContinue) {
 				...
 				//convert a js objet to string to send in to the web server
 				var myJSON = JSON.stringify(myProductsId);
 				var params = "products=" + myJSON + "&quantity=" + quantity;
 				var xmlh = new XMLHttpRequest();
 				xmlh.onreadystatechange = function() {
 					if (this.readyState == 4 && this.status == 200) {
 						alert("Product added !");
 						document.getElementById("MyBagDivToAppId").innerHTML = this.responseText;
 						window.location.reload();
 					}
 				}
 				xmlh.open("POST", "addProductToMySession.php", true);
