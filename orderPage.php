...
<body style="margin:0px;overflow-x:hidden;">
	...
	<div class="orderPageOrderDiv">
		<h2>Order</h2>
		<div class="orderPageOrderDivAllProductsDiv">
			<div class="orderPageOrderDivAllProductsDivUp">
				<?php
				$con = mysqli_connect("localhost", "root", "", "web3") or die("unable to connect");

				for ($i = 0; $i < count($_SESSION['AllMyProducts']); $i++) {
					$quer = "SELECT * FROM products WHERE id = '" . $_SESSION['AllMyProducts'][$i] . "'";
					$res = mysqli_query($con, $quer);
					$rslt = mysqli_fetch_assoc($res);

					echo "<div class='orderPageOrderDivProductsDiv' onclick='checkThisProduct(" . $_SESSION['AllMyProducts'][$i] . ")'>";
					echo "<div class='orderPageOrderDivProductsDivImgDiv'>";
					echo "<img src='images/" . $rslt['image'] . "' class='orderPageOrderDivProductsDivImg' /></div>";
					echo "<div class='orderPageOrderDivProductsDivTxtDiv'>";
					echo "<label>Name : " . $rslt['name'] . "</label><br>";
					echo "<label>Price : " . $rslt['price'] . "</label><br>";
					echo "<label>Quantity : " . $_SESSION['quantityOfEachProduct'][$i] . "</label>";
					echo "</div>";
					echo "</div>";
				}
				mysqli_close($con);
			...
			<div class="orderPageOrderDivAllProductsDivDown">
				<div class="orderPageOrderDivAllProductsDivDownLeft">Total : </div>

				<div class="orderPageOrderDivAllProductsDivDownRight">
					<?php
					echo $_SESSION['totalPrice'] . " L.L.";
					?>
				</div>
			</div>
		...
		<div class="orderPageOrderDivOrderDetails">
			<div class="orderPageOrderDivOrderDetailsDivOfLine">
				<div class="orderPageOrderDivOrderDetailsLabelType">Type of Payment : </div>
				<div class="orderPageOrderDivOrderDetailsSelectDiv">
					<select id="paymentTypeId" class="inputSizeOrderDetails" name="paymentType">
						<option id="paymentTypeCardId" value="card">Card</option>
						<option id="paymentTypeCashId" value="cash">Cash</option>
					</select>
				</div>
			</div>
			<br><br>
			<div class="orderPageOrderDivOrderDetailsDivOfLine">
				<div class="orderPageOrderDivOrderDetailsLabelType">Card number : </div>
				<div class="orderPageOrderDivOrderDetailsSelectDiv">
					<input type="number" id="cardNumberId" name="cardNumber" />
				</div>
			</div>

			<br><br>
			<div class="orderPageOrderDivOrderDetailsDivOfLine">
				<div class="orderPageOrderDivOrderDetailsLabelType">Address : </div>
				<div class="orderPageOrderDivOrderDetailsSelectDiv">
					<?php
					$con = mysqli_connect("localhost", "root", "", "web3") or die("unable to connect");
					...
					if (mysqli_num_rows($res) > 0) {
						$rslt = mysqli_fetch_assoc($res);
						echo "<input id='addressIdHidden' class='inputSizeOrderDetails' name='addressName' hidden value=\"" . $rslt['id'] . "\">";
						echo "<input id='addressId' class='inputSizeOrderDetails' name='addressName' readonly value=\"" . $rslt['city'] . "\">";
					}
					mysqli_close($con);
					?>

					<button id="EdtNewAddressId" class="addAddressBtn">Edit</button>
				</div>
			</div>
			...
			<div class="privacyTerms">
				<div class="privacyTermsSecond">Your personal data will be used to process your order, support your experience throughout this website, and for other purposes described in our privacy policy.</div>
			</div>

			<br><br>

			<div class="makeOrderDiv">
				<button class="makeOrderBtn" onclick="makeOrder();">Make It</button>
			</div>
		</div>
	...
	<script>
		...
		$("#EdtNewAddressId").click(function() {
			window.location.href = "editLocation.php?addressId=" + <?php echo $rslt['id']; ?>;
		});
		...
		$("#paymentTypeId").on("change", function() {
			...
			if (this.value == "cash") {
				$("#cardNumberId").attr("disabled", "disabled");
			} else {
				$("#cardNumberId").removeAttr("disabled");
			}
		});
		...
		function makeOrder() {
			var typePaym = document.getElementById("paymentTypeId").value;
			var cardNumb = document.getElementById("cardNumberId").value;
			var addrss = document.getElementById("addressIdHidden").value;
			var canContinue = true;


			if (typePaym == "card") {
				if (cardNumb.length == 0 || addrss.length == 0) {
					canContinue = false;
				}
			} else {
				if (addrss.length == 0) canContinue = false;
			}

			if (canContinue) {
				ordMyProductsId = JSON.parse('<?php
												if (isset($_SESSION['AllMyProducts'])) {
													echo json_encode($_SESSION['AllMyProducts']);
												} ?>');

				if (ordMyProductsId.length == 0) {
					alert("You must have minimum one product");
					canContinue = false;
				}
				...
				if (!canContinue) {
					...
					<?php echo "ordMyProductsTotal = '" . $_SESSION['totalPrice'] . "';" ?>
					// alert(ordMyProductsId+"  "+ordMyProductsTotal);
					...
					// var params = "";
					var params = "amount=" + ordMyProductsTotal + "&code=" + cardNumb + "&status=" + typePaym;
					xmlhttp.open("POST", "addOrderFunction.php", true);
