  ...
  <body style="margin:0px;overflow-x:hidden;">
  	...
  	<?php
		...
		$query = "SELECT * FROM products WHERE id = '" . $_SESSION['prodIdSelected'] . "'";
		$res = mysqli_query($con, $query);
		$fetch = mysqli_fetch_assoc($res);

		$queryQtt = "SELECT Quantity FROM stock WHERE product_id = '" . $_SESSION['prodIdSelected'] . "'";
		$resQtt = mysqli_query($con, $queryQtt);
		$fetchQtt = mysqli_fetch_assoc($resQtt);
		// -->
		?>

  	<div class="adminSelectedProdDiv">
  		<div class="adminSelectedProdDivLeft">
  			<img src="images/<?php echo $fetch['image']; ?>" class="selectedProductDivLeftImg" />
  		</div>
  		<div class="adminSelectedProdDivRight">
  			<div class="adminSelectedProdDivRightName">
  				<div class="adminSelectedProdDivRightNameLeft">name : </div>
  				<div class="adminSelectedProdDivRightNameRight"><textarea id="adminPrdctSlctdNameId" class="adminSelectedProdDivRightNameTxtArea"><?php echo $fetch['name']; ?></textarea></div>
  			</div>
  			<div class="adminSelectedProdDivRightDesc">
  				<div class="adminSelectedProdDivRightNameLeft">description : </div>
  				<div class="adminSelectedProdDivRightNameRight"><textarea id="adminPrdctSlctdDescId" class="adminSelectedProdDivRightNameTxtArea"><?php echo $fetch['description']; ?></textarea></div>
  			</div>
  			<div class="adminSelectedProdDivRightPric">
  				<div class="adminSelectedProdDivRightNameLeft">price : </div>
  				<div class="adminSelectedProdDivRightNameRight"><textarea id="adminPrdctSlctdPricId" class="adminSelectedProdDivRightNameTxtArea"><?php echo $fetch['price']; ?></textarea></div>
  			</div>
  			<div class="adminSelectedProdDivRightQtt">
  				<div class="adminSelectedProdDivRightNameLeft">quantity : </div>
  				<div class="adminSelectedProdDivRightNameRight"><textarea id="adminPrdctSlctdQttId" class="adminSelectedProdDivRightNameTxtArea"><?php echo $fetchQtt['Quantity']; ?></textarea></div>
  			</div><br>
  			<div class="adminSelectedProdDivRightArch">
  				<div class="adminSelectedProdDivRightNameLeft">Archived :</div>
  				<?php
					if ($fetch['archived_at'] != NULL) { ?><input type="checkbox" id="adminPrdctSlctArchId" style="height:20px;width:20px;margin:0px;" checked onclick="adminChangeCheckBoxPrdct();" />
  				<?php } else { ?><input type="checkbox" id="adminPrdctSlctArchId" style="height:20px;width:20px;margin:0px;" onclick="adminChangeCheckBoxPrdct();" />
  				<?php } ?>
  			</div>
  			<div style="text-align:center;"><button class="adminSavePrdctBtn" onclick="adminSavePrdct('<?php echo $fetch['id']; ?>');">Save</button></div>
  			<br>
  		</div>
  	</div>
  	...
  	<script>
  		// <button class="adminDeletePrdctBtn" onclick="adminRemoveProd('<?php echo $fetch['id']; ?>')";>Delete</button>
